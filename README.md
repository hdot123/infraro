...
消费仓接入按语言栈各取一套模板，两栈内容对齐、锚点一致（全部 `@v0.19.0`）：
...
| 类别 | 文件 | 形态 |
|------|------|------|
| thin-caller | `evolution-scan.yml` / `evolution-heartbeat.yml` / `auto-merge.yml` / `droid-review.yml` / `droid-review-watchdog.yml` | `uses@v0.19.0` tag 锚 |
...
- [Workflow Templates](templates/) — 双栈目录 `templates/python/` 与 `templates/typescript/`（@v0.19.0）；根下单栈历史存档已删除（陈旧 @v0.18.5 形态，双栈为其 supersede）
