...
版本 tag 遵循 `vMAJOR.MINOR.PATCH`（当前主线 `v0.18.x`，`v0.18.7` = commit `3fc1cbc`）。所有下游锚点引用该 tag（见 3.2 锚点纪律）。
...
- `scanner_workflow` 输入为引擎侧预留（覆盖心跳引擎探测的扫描器文件名）；`v0.18.7` 声明模板不传该输入，消费仓必须保持契约文件名 `evolution-scan.yml` / `evolution-heartbeat.yml`（见 3.2）。
...
| 文件 | 形态 | 锚点形态 |
| --- | --- | --- |
| `evolution-scan.yml` | thin-caller | `uses@v0.18.7` |
| `evolution-heartbeat.yml` | thin-caller | `uses@v0.18.7` |
| `droid-review.yml` | thin-caller（分片流水线委托引擎，聚合 job 留本地） | reusable `@v0.18.7` + 聚合 action SHA pin |
| `droid-review-watchdog.yml` | thin-caller（双 handler 委托 + 本地 quota-sweep） | `uses@v0.18.7` |
| `auto-merge.yml` | thin-caller | `uses@v0.18.7` |
...
| `.evolution/config.yml` | evolution 配置（含 `engine_ref: v0.18.7` 锚） |
...
- 单一 tag：同一交付周期内，全部 reusable workflow 引用统一锚同一引擎 tag（当前 `v0.18.7`）；有效锚点（`uses@tag` 与 `engine_ref`）经 grep 只允许出现一个版本号。
...
| 5 | 匿名 install 实测 | 无 token 执行 `pip install git+https://github.com/hdot123/infraro-core.git@v0.18.7` 成功 |
